...
if (process.env.IS_LAMBDA_IMAGE_BUILD === true) {
  const module = await import('chrome-aws-lambda');
...
export async function create(puppeteerArgs) {
  ...
  const launchOptions = {
    headless: chromium ? chromium.headless : other.browserShellMode ? 'shell' : true,
    userDataDir: puppeteerOptions.tempDir || './tmp/',
    args: chromium ? chromium.args : puppeteerArgs,
