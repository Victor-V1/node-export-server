...
import {
  ...
  CHART_AGGREGATOR_OPERATOR_DETAILS
} from './constants.js';

import { CommonStatsType } from './enums.js';
dayjs.extend(utc);
...
export function convertSecondsIntoMinuteAndSecondsString(seconds) {
  if (seconds === undefined) {
    return 'N/A';
  }
  const minutes = Math.floor(seconds / 60);
  const secondsRemaining = seconds % 60;
  return `${minutes}m ${Math.round(secondsRemaining)}s`;
}
...
function mapChartMetricsValueToSubtitleString(
  ...
  if ('forSubcategory' in metric) {
    ...
    return previewMode
      ? Object.entries(lodash.countBy(metricLabels)).map(
          ([metricName, count]) =>
