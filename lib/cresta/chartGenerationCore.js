...
const CSV_DELIMITER = ','; // Default, adjust if different
...
function mapChartAttributeValueToString(attributeValue, chartDataMaps) {
  if (!attributeValue || !chartDataMaps) {
    return ''; // Fallback
  }
  if (attributeValue.stringValue !== undefined)
    return attributeValue.stringValue;
  if (attributeValue.intValue !== undefined)
    return String(attributeValue.intValue);
  if (attributeValue.boolValue !== undefined)
    return String(attributeValue.boolValue);
  if (attributeValue.timestamp)
    return new Date(attributeValue.timestamp).toLocaleDateString(); // Example formatting

  if (
    attributeValue.subcategoryValue &&
    attributeValue.subcategoryValue.momentName
  ) {
    const moment =
      chartDataMaps.moments &&
      chartDataMaps.moments[attributeValue.subcategoryValue.momentName];
    return moment
      ? moment.displayName
      : attributeValue.subcategoryValue.momentName;
  }
  if (attributeValue.usecaseNameValue && chartDataMaps.usecases) {
    const usecase = chartDataMaps.usecases[attributeValue.usecaseNameValue];
    return usecase ? usecase.displayName : attributeValue.usecaseNameValue;
  }
  // Add more conditions for other types like agentValue, teamValue, metadataAttributeValue, etc.
  // Example for metadataAttributeValue:
  // if (attributeValue.metadataAttributeValue && chartDataMaps.metadataAttributes) {
  //   const metaAttr = chartDataMaps.metadataAttributes[attributeValue.metadataAttributeValue.attributeName];
  //   if (metaAttr) {
  //     const foundVal = metaAttr.allowedValues.find(v => v.value === attributeValue.metadataAttributeValue.value.stringValue); // or other types
  //     return foundVal ? foundVal.displayName : attributeValue.metadataAttributeValue.value.stringValue;
  //   }
  // }
  return JSON.stringify(attributeValue); // Fallback for unhandled types
}
